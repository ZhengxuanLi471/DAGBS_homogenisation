...
def get_or_create_vertex(verts_list: List[List[float]], vert_map: Dict[Tuple, int],
                         x: float, y: float, z: float, eps: float = 1e-5) -> int:
    """Get existing vertex index or create new one.

    Uses spatial hashing with neighbor-cell probing to robustly merge
    nearby vertices despite floating-point rounding boundary effects.
    """
    ix, iy, iz = round(x / eps), round(y / eps), round(z / eps)
    # Check primary cell and all 26 neighbors to avoid boundary misses
    for dx in (-1, 0, 1):
        for dy in (-1, 0, 1):
            for dz in (-1, 0, 1):
                probe = (ix + dx, iy + dy, iz + dz)
                if probe in vert_map:
                    oidx = vert_map[probe]
                    ox, oy, oz = verts_list[oidx]
                    if abs(ox - x) < eps and abs(oy - y) < eps and abs(oz - z) < eps:
                        return oidx
    key = (ix, iy, iz)
    idx = len(verts_list)
