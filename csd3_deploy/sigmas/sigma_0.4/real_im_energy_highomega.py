# DAGBS HIGH-OMEGA TAIL driver. Companion to real_im_energy.py: it sweeps only the
# extra decades ln(omega*tau_M) in (10, 15] so the existing [-3, 10] CSVs can be
# extended up into the unrelaxed (normal-locked elastic) plateau WITHOUT recomputing
# them. Output goes to a separate "_highomega" CSV; concatenate later (drop nothing
# -- the grids don't overlap; the first high point == 10 + step, the existing max
# point being exactly 10).
#
# One deliberate difference from real_im_energy.py:
#   Grid: 39 points on the SAME spacing as linspace(-3,10,100) (h=13/99),
#   running from 10.1313 up to 15.1212 (strictly above 10, so no duplicate).
# solver='cg' (the real_im_energy.py default): unlike the low-omega tail (which
# needs Pardiso because CG floors the tiny C' ~ omega^2 storage), the high-omega
# tail sits on the unrelaxed plateau where the GB-diffusion block (i*C_d/omega) is
# small and the system tends to the well-conditioned normal-locked elastic limit --
# CG converges fine there and direct (Pardiso) is far too slow for 39 solves/seed.
...
parser = argparse.ArgumentParser(
    description="High-omega tail (ln omega*tau_M in (10,15]) for a range of seeds"
)
...
# High-omega grid on the SAME spacing as the original linspace(-3, 10, 100).
_H = 13.0 / 99.0                                  # original step
_N_HIGH = int(np.ceil(5.0 / _H))                 # 39 points to reach ~15
LN_OMEGA_HIGH = 10.0 + _H * np.arange(1, _N_HIGH + 1)   # ascending, > 10
...
def run_branch(Gamma, gamma_tag, mesh, spaces, contact_pairs, outer_contact_pairs,
    ...
    """Sweep the high-omega grid for one macro loading tensor and dump energy curves."""

    print("Starting with single Gamma:", Gamma)
    ln_omega = LN_OMEGA_HIGH
    ...
    for j in range(len(ln_omega)):
        ...
        gfu, mesh = solve_rve(
            ...
            mu=MU,
            omega=omegai,
            solver='cg',              # high-omega plateau is well-conditioned; CG (direct too slow)
            rtol=1e-8,
    ...
    out_path = 'Seed_{}_energy_real_im_data_{}_highomega.csv'.format(seedname, gamma_tag)
    df.to_csv(out_path, index=False)
    print(f"Saved high-omega energy curves to {out_path}")
...
for seed in seeds:
    ...
    # Same per-seed tau_M calibration as real_im_energy.py, so the high-omega axis
