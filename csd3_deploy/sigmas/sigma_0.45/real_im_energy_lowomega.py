...
#
# Two deliberate differences from real_im_energy.py:
#   1. Grid: 23 points on the SAME spacing as linspace(-3,10,100) (h=13/99),
#      running from -6.0202 up to -3.1313 (strictly below -3, so no duplicate).
#   2. solver='direct'. At low omega the storage modulus C' ~ omega^2 is tiny and
#      CG re-floors it at the residual level (CLAUDE.md), which corrupts
#      G_ss = G2^2/G1 and hence Delta. Direct (Pardiso) resolves the true tail,
#      which is the whole point of measuring the low-omega plateau.
...
def run_branch(Gamma, gamma_tag, mesh, spaces, contact_pairs, outer_contact_pairs,
    ...
    for j in range(len(ln_omega)):
        ...
        gfu, mesh = solve_rve(
            ...
            mu=MU,
            omega=omegai,
            solver='direct',          # low-omega C' tail: CG floors it, use Pardiso
            rtol=1e-8,
